  ...
  return (
    <div className="w-full h-full overflow-y-auto animate-in fade-in duration-500 relative" style={{ backgroundColor: gameBackgroundColor }}>
      ...
      <div
        ...
        style={{
          top: 'max(0.75rem, calc(env(safe-area-inset-top, 0.5rem) + 0.5rem))',
        }}
      >
        <img 
      ...
      <div 
        ...
        style={{ 
          paddingTop: 'max(4rem, calc(env(safe-area-inset-top, 0.5rem) + 0.5rem + 2rem + 1rem))',
        }}
      >
        ...
        {/* Stats Cards - Side by side on mobile landscape */}
        <div className={`w-full max-w-6xl grid grid-cols-2 gap-2 sm:gap-3 md:gap-4 transition-all duration-1000 delay-300 ${showStats ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="bg-gray-50 border-2 border-gray-300 p-2 sm:p-3 md:p-4 relative overflow-hidden group hover:border-yellow-500 transition-all rounded-lg">
            ...
            <div className="relative flex flex-col h-full">
              ...
              <button
                onClick={onRestart}
                className="group/btn relative px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2.5 bg-black text-white text-[9px] sm:text-[10px] md:text-xs overflow-hidden transition-all hover:scale-105 active:scale-95 whitespace-nowrap rounded-lg font-medium min-h-[36px] sm:min-h-[40px] flex items-center justify-center mt-auto"
              >
              ...
              <form onSubmit={handleSaveScore} className="space-y-1.5 sm:space-y-2 flex-1 flex flex-col">
                <div className="flex gap-1 sm:gap-1.5">
                  <input
                    ...
                    maxLength={3}
                    className="flex-1 px-2 py-1.5 sm:px-2.5 sm:py-2 text-[10px] sm:text-xs md:text-sm bg-white border-2 border-gray-300 text-black placeholder-gray-400 focus:border-yellow-500 focus:outline-none rounded-lg min-h-[36px] sm:min-h-[40px]"
                    disabled={isSaving}
                    autoFocus
                  />
                  <button
                    ...
                    className="px-2 py-1.5 sm:px-3 sm:py-2 text-[10px] sm:text-xs md:text-sm bg-yellow-500 text-black hover:bg-yellow-400 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-all rounded-lg font-medium min-h-[36px] sm:min-h-[40px] whitespace-nowrap"
                  >
                    {isSaving ? '...' : 'SAVE'}
                  </button>
                </div>
                <input
                  ...
                  className="w-full px-2 py-1.5 sm:px-2.5 sm:py-2 text-[10px] sm:text-xs md:text-sm bg-white border-2 border-gray-300 text-black placeholder-gray-400 focus:border-yellow-500 focus:outline-none rounded-lg min-h-[36px] sm:min-h-[40px]"
                  disabled={isSaving}
                />
                {email.trim() && !isValidEmail(email) && (
        ...
        {/* Leaderboard Container - Full width below */}
        <div className={`w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 md:gap-6 transition-all duration-1000 delay-300 ${showStats ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          {/* Leaderboard - Full width on mobile, half on larger screens */}
